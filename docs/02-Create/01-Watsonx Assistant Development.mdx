...
4. Click "Invoke watsonx generation API", and click step 1.
   ![](../../assets/screenshots/watsonx_assistant_watsonxai_action.png)
5. Click "Edit extension" to setup watsonx.ai custom extension.
6. From the Extension options, select "watsonx.ai". From Operation options, select "Generation".
7. For "Parameters" 1. set **version** to `watsonx_api_version` variable 1. set **input** to `model_input` variable 1. set **model_id** to `model_id` variable 1. set **project_id** to `watsonx_project_id` variable
