...
1. In Authentication page, in the Authentication type dropdown, select "Basic auth"
    ...
    1. You'd see the following view.
        ...
        1. Copy **URL** and paste it in *discovery_url* and remove `https://` from the beginning of the string
...
1. Click "Invoke watsonx generation API", and click step 1.
![](../../assets/screenshots/watsonx_assistant_watsonxai_action.png)
1. Click "Edit extension" to setup watsonx.ai custom extension. 
1. From the Extension options, select "watsonx.ai". From Operation options, select "Generation".
1. For "Parameters"
    1. set **version** to `watsonx_api_version` variable
    1. set **input** to `model_input` variable
    1. set **model_id** to `model_id` variable
    1. set **project_id** to `watsonx_project_id` variable
